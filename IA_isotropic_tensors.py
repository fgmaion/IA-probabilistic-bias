...
def round_expr(expr, num_digits):
    return expr.xreplace({n : round(n, num_digits) for n in expr.atoms(sympy.core.numbers.Number)})

class IsotropicTensor():
    def __init__(self, numeric_rep, symbol=None, symbol_com=None, symshape=None):
        """
        ...
        symshape : the symmetries of the tensor e.g. (2,2,2)
        """
    ...
    def inner_product(self, other):
        ...
        res = np.sum(self.N()*on)

        try:
            res.dtype==np.float64
            return rounded(res)
        except:
            return simplify(nsimplify(round_expr(res, 8)))

        #return np.sum(self.N()*on)
...
class IsotropicTensorSystem():
    def __init__(self, maxorder=2, verbose=False, usecache=True):
        ...
        def create_tens(perm, symgroup, symbol="x", sym_equal_len_groups=True):
            ...
            return IsotropicTensor(num, symbol=symb, symbol_com=symb_com, symshape=symshape)
        
        self.sigma = sympy.symbols("sigma:3", real=True, positive=True)
        self.x = sympy.Matrix([sympy.symbols("T R S", commutative=False)]).T
        self.I = sympy.Matrix([sympy.symbols("I", commutative=False)]).T
        ...
        basis[""]["I0"] = IsotropicTensor(np.array(1), symbol=sympy.Number(1), symshape=())
        ...
        obasis[""]["J0"] = IsotropicTensor(np.array(1), symbol=sympy.Number(1), symshape=())
        ...
        # 6 Tens
        symgroup = ((0,1), (2,3), (4,5))
        basis["222"]["I222"] = create_tens("ijklmn", symgroup, symbol="I_222", sym_equal_len_groups=True)
        basis["222"]["I2=22"] = create_tens("ikjlmn", symgroup, symbol="I_2=22", sym_equal_len_groups=False)
        basis["222"]["I22=2"] = create_tens("ijkmln", symgroup, symbol="I_22=2", sym_equal_len_groups=False)
        basis["222"]["I222="] = create_tens("njklmi", symgroup, symbol="I_222=", sym_equal_len_groups=False)
        basis["222"]["I2-2-2-"] = create_tens("iklmnj", symgroup, symbol="I_2-2-2-", sym_equal_len_groups=True)
        
        obasis["222"]["J222"], obasis["222"]["J2=22"], obasis["222"]["J22=2"], obasis["222"]["J222="], obasis["222"]["J2-2-2-"] = \
            orthogonalize([basis["222"][k] for k in basis["222"]], symbols=("J_222", "J_2=22", "J_22=2", "J_222=", "J_2-2-2-"))
    ...
    def symbol_JtoI(self, basis_name=None, limit_denom=10**8):
        ...
        if basis_name is not None:
            ...
            for bname in self.obasis:
                res.extend(self.symbol_JtoI(bname, limit_denom=limit_denom))
            return res
    ...
    def coeffrep_multidot(self, A, *crepBs, products=(), offset=0):
        """returns A multiple times multiplied and / or contracted with differnt B
    ...
    def IA_symbol_bias_deriv1(self, term=None, potderiv=(2,3,4)):
        """This term (f) is defined so that b1* = < f >_galaxies
        ...
        def single_term(term, component):
            ...
            for i in range(0, nderiv):
                ...
                res += self.coeffrep_to_symbol(*crep) * x[i] * I / self(term).norm2()
            return sympy.together(res)
        ...
        if term is None: # return all terms
            v = Ci*x
            for comp, deriv in enumerate(potderiv):
                for tname in self.obasis["2%d" % deriv]:
                    res[tname] = single_term(tname, comp)
            return res
        else:
            for comp, deriv in enumerate(potderiv):
                if term in self.obasis["2%d" % deriv]:
                    return single_term(term, comp)
            return 0
        
    def IA_symbol_bias_deriv2(self, term=None, potderiv=(2,3,4)):
        """This term (f) is defined so that b2* = < f >_galaxies
        ...
        def single_term(term, c1, c2):
            ...
            for i in range(0, nderiv):
                for j in range(0, nderiv):
                    crep = self.coeffrep_multidot(self(term), Ci_coeffrep[c1][i], Ci_coeffrep[c2][j], products=(1,1))
                    res += self.coeffrep_to_symbol(*crep) * x[i] * x[j] * I[0] / norm2
                    
            crep = self.coeffrep_multidot(self(term), Ci_coeffrep[c1][c2], products=(2,))
            res += - self.coeffrep_to_symbol(*crep) * I[0] / norm2
    ...
    def IA_symbol_bias_derivN(self, term, potderiv=(2,), getderiv=False):
        if term in ("J22", "J2=2", "J3-3", "J3---3", "J44", "J4=4", "J4==4", "J24", "J2=4"):
            deriv, term = 1, self.IA_symbol_bias_deriv1(term, potderiv=potderiv)
        elif term in ("J222", "J2=22", "J22=2", "J222=","J2-2-2-"):
            deriv, term = 2, self.IA_symbol_bias_deriv2(term, potderiv=potderiv)
        else:
            raise NotImplementedError(term)
        if getderiv:
            return term, deriv
    ...
    def IA_pycode_bias_estimator(self, term, potderiv=(2), label="o2", getinfo=False):
        import re

        t,deriv = self.IA_symbol_bias_derivN(term, potderiv=potderiv, getderiv=True)
